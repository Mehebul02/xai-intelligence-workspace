...
/** Synthetic "observed" series with a deliberate spike after index 26. */
...
/** Flat-growth baseline series, used as a visual reference against `observed`. */
...
function toPath(values: number[]): string {
  return values
    .map((v, i) => `${i === 0 ? "M" : "L"}${(i / (values.length - 1)) * 100},${80 - (v / 90) * 70}`)
    .join(" ");
}

export function BigChart() {
  const observed = buildObservedSeries(POINT_COUNT);
  const baseline = buildBaselineSeries(observed);
